...
def train(train_loader,vae_net, data_logger,optimizer, device, epoch):
    vae_net.train()
...
def test(test_images, vae_net, data_logger, optimizer, device):
    vae_net.eval()
    with torch.no_grad():
        with torch.cuda.amp.autocast():
...
if __name__ == '__main__':
    ...
    for epoch in trange(start_epoch, args.nepoch, leave=False):
        train(train_loader,vae_net, data_logger,optimizer, device, epoch)
        vae_net.train()
        ...
        if epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1:
            ...
            test(test_images, vae_net, data_logger, optimizer, device)
